...
export function InvoiceC({ type = "TAX INVOICE" }: { type?: string }) {
    ...
    <div className="doc-page" style={{ background: WH, fontFamily: "'Helvetica Neue', Arial, sans-serif" }}>
      ...
      <div style={{ padding: "14px 44px 0" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div>
            <div style={{ fontSize: 28, fontWeight: 900, color: NAVY, letterSpacing: "0.04em" }}>{type}</div>
            <div style={{ fontSize: 11, color: RED, fontWeight: 600, letterSpacing: "0.18em", marginTop: 2 }}>{isTax ? "ใบกำกับภาษี" : "ใบแจ้งหนี้"}</div>
            {isTax && <div style={{ fontSize: 9, color: "#aaa" }}>ต้นฉบับ / Original</div>}
          </div>
          <div style={{ background: "#f9f9f9", borderTop: `3px solid ${RED}`, border: `1px solid #eee`, padding: "10px 18px", fontSize: 10, lineHeight: 2 }}>
            <div><span style={{ color: "#bbb", width: 110, display: "inline-block" }}>เลขที่ / No.:</span><strong style={{ color: NAVY }}>{docNo}</strong></div>
            <div><span style={{ color: "#bbb", width: 110, display: "inline-block" }}>วันที่ / Date:</span>{docDate}</div>
            {isTax && <div><span style={{ color: "#bbb", width: 110, display: "inline-block" }}>ครบกำหนด / Due:</span>30 วัน</div>}
            <div><span style={{ color: "#bbb", width: 110, display: "inline-block" }}>อ้างอิง / Ref.:</span>TSE-QT-XXXX</div>
          </div>
        </div>

        <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
          {[["BILL TO / เรียน", NAVY, ["ชื่อบริษัท / Company", "ที่อยู่ / Address", "เลขผู้เสียภาษี / Tax ID", "ผู้ติดต่อ / Contact"]], ["PROJECT / โครงการ", RED, ["ชื่อโครงการ / Project", "PO No.", "วันที่สั่งซื้อ / PO Date", "จัดส่ง / Ship To"]]].map(([title, color, fields]) => (
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 10 }}>
          <div style={{ width: 290 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "9px 14px", fontWeight: 800, fontSize: 13, background: `linear-gradient(90deg, ${NAVY}, #0d1a36)`, color: WH, marginTop: 4 }}>
        ...
        <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
          <div style={{ borderLeft: `3px solid ${RED}`, paddingLeft: 12 }}>
            <div style={{ fontSize: 8.5, fontWeight: 700, color: RED, letterSpacing: "0.14em", marginBottom: 8 }}>ชำระโดย / PAYMENT</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 16px" }}>
            {["ผู้จัดทำ / Prepared", "ผู้อนุมัติ / Approved"].map(label => (
              <div key={label} style={{ textAlign: "center" }}>
                ...
                <div style={{ fontSize: 8.5, color: NAVY, fontWeight: 700 }}>{label}</div>
